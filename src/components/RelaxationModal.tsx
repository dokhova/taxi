...
export function RelaxationModal({ isOpen, onClose }: RelaxationModalProps) {
  ...
  }, [isOpen, isPlaying, duration, isRepeat]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative w-full max-w-3xl bg-black/40 backdrop-blur-2xl rounded-xl shadow-2xl overflow-hidden p-4 md:p-6 border border-white/10 max-h-[90vh] overflow-y-auto"
      >
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-10 h-10 bg-black/60 hover:bg-black/80 backdrop-blur-xl rounded-full flex items-center justify-center transition-colors border border-white/20"
        >
          <X className="w-5 h-5 text-white/90" />
        </button>

        <div className="flex flex-col md:flex-row gap-4 md:gap-6 pt-8 md:pt-0">
          {/* Left side - Sessions list */}
          <div className="flex-1 space-y-2 md:max-w-md pr-8 md:pr-0">
            {/* Meditation */}
            <motion.button
